...
LOCK_FILE="${INVOICE_FLOW_SYNC_LOCK_FILE:-/run/lock/invoice-flow-operational-sync.lock}"
VERBOSITY="${INVOICE_FLOW_SYNC_VERBOSITY:-1}"
...
on_error() {
    ...
    log "SYNC RESULT=ERROR EXIT_CODE=${exit_code} LINE=${line_number} DURATION_SECONDS=${elapsed}"
    exit "${exit_code}"
}
...
if ! flock -n 9; then
    log "SYNC RESULT=SKIPPED REASON=ALREADY_RUNNING"
    exit 0
fi
...
log "Ejecutando sincronización operacional dentro del contenedor."
...
docker compose \
    ...
    -T \
    "${DJANGO_SERVICE}" \
    python manage.py sync_operational_data \
    --verbosity "${VERBOSITY}" \
    --no-color
...
log "===================================================================="
log "SYNC RESULT=SUCCESS EXIT_CODE=0 DURATION_SECONDS=${ELAPSED}"
log "===================================================================="